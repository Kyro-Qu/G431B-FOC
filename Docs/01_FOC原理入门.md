...
> **本文是"概念版"**。想看到每一步的**数学推导 + 公式 + 逐行代码**，
> 去读 [00 · 底层原理](00_底层原理_从变换到SVPWM与快环数据流.md)
> （Clarke/Park/SVPWM/采样链/快环数据流/参数映射，全推导）。
...
> 公式的完整推导（为什么系数是 √3 不是 2/3、怎么用 a+b+c=0 消去 c、
> 等幅值 vs 等功率）见 [00 篇 §1](00_底层原理_从变换到SVPWM与快环数据流.md)。
...
> 想知道"中点注入为什么和教科书七段式 SVPWM 数学等价"（伏秒平衡解出
> T1/T2 → 映射到占空比）、"母线利用率高 15.47% 是怎么算出来的"、
> "6%/94% 钳位为什么和采样窗口绑死"——见 [00 篇 §3](00_底层原理_从变换到SVPWM与快环数据流.md#3-svpwm把电压矢量变成三相占空比)。
...
本工程默认 ω = 2000 rad/s（约 318 Hz），在 `foc_motor_init()` 里自动算好。
你只需要填对电机的 R 和 L（[foc_config.h](../MDK-ARM/Code/foc/HAL/foc_config.h)）。

> 完整推导（为什么 Kp=Ls·ω、Ki=Rs·ω 能对消极点；本板 0.1 Ω/20 µH
> 对应的 Kp/Ki 实算值；以及"为什么不能把辨识的 336 µH 直接当电流环
> 设计参数"）见 [00 篇 §6](00_底层原理_从变换到SVPWM与快环数据流.md#6-底层参数映射配置里的数字从哪来)。
...
| **VESC** | 中点注入 SVPWM 写法；全部控制放中断里的确定性调度；故障码集中管理 + 终端命令行调试；磁链观测器算法（已随无感调试台接入主工程快环，调试中，文档未展开） | `foc_svm.c`、`foc_motor_fast_loop()`、`foc_cmd.c`、`foc_observer.c` |
| **ST MCSDK** | 三电阻低边采样的窗口规划、自举电容预充电、ADC 注入队列时序（这部分在本板硬件上验证过） | `current_shunt.c`、`foc_board_g431.c` |
| **MESC / moteus** | 调研对象：MESC 的 HFI 低速无感、moteus 的高级位置控制与热模型是后续路线图（见 09 篇） | 暂未落地，思路记录在文档 |
