...
| [Docs/04_上手指南.md](Docs/04_上手指南.md) | 编译烧录、逐层验证流程、调参顺序、故障对照表 |
...
烧录后：串口 6.5 Mbaud 发 `help` 看命令；按板上按键开环旋转；
`id` 自动测电机参数 → `m vel` → 按键校准 → 按键运行 → `t 1000` 转 1000 RPM。
